...
- **(ocr): the candle OCR backends no longer repeat one phrase to the end of a page.** DeepSeek-OCR and PaddleOCR-VL chose every token with a bare argmax and no repetition control, so once the decoder reached a state that reproduced itself it emitted the same text until it reached the token limit: one dense page came back as 11,679 characters with almost nothing distinct in them. All three vision-language backends now choose the next token through one shared step built on candle's own generation API, which applies a repetition penalty first. The same page returns 1,797 characters in a fifth of the time. The penalty has two named forms: DeepSeek-OCR and PaddleOCR-VL lower a repeated token's score once, and GLM-OCR lowers it once for every time the token has appeared, over its whole output, which is how GLM-OCR has always decoded. GLM-OCR also no longer fails on BF16 weights. PaddleOCR-VL's token limit now counts generated tokens instead of the prompt and the output together. (GH#1674)
