...
- **(ocr): the DeepSeek-OCR page limit is configurable instead of a fixed 128 tokens.** The decode loop stopped after 128 new tokens whatever the page held, so a dense page came back truncated at roughly 400 to 650 characters and the backend read as though it could not see most of the text. The limit is now a model configuration field, defaulting to 4096 in line with the other full-page OCR backend. Raising it exposes a separate defect in the same loop: at F32 the backend repeats one phrase until it reaches the limit, which the old 128-token cut-off had been hiding. That is tracked as GH#1701. (GH#1674)
- **(ocr): the candle OCR backends apply a repetition penalty when choosing each token.** DeepSeek-OCR and PaddleOCR-VL chose every token with a bare argmax and no repetition control, so a decoder that reached a state reproducing itself had nothing to push it out. All three vision-language backends now choose the next token through one shared step built on candle's own generation API, which applies the penalty first. The penalty has two named forms: DeepSeek-OCR and PaddleOCR-VL lower a repeated token's score once, and GLM-OCR lowers it once for every time the token has appeared, over its whole output, which is how GLM-OCR has always decoded. GLM-OCR also no longer fails on BF16 weights. PaddleOCR-VL's token limit now counts generated tokens instead of the prompt and the output together. This does not stop DeepSeek-OCR repeating to the end of a page at F32: that is a defect in the backend rather than in token selection, and it is tracked as GH#1701. (GH#1674)
